...
async def ban_channel(message: types.Message):
    ...
    admin_mentioned = from_user.get_mention()
    ...
    try:
        ...
        logger.info(f"Канал {member_fullname} был забанен админом {admin_fullname}")
    except BadRequest:
        logger.info(f"Бот не смог забанить канал {member_fullname}")
